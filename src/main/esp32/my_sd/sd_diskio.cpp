...
//
// Modifications by roman-ks, 2025
// exposed rawRead/Write methods with sector count parameter
...
bool sd_read_raw(uint8_t pdrv, uint8_t *buffer, DWORD sector, size_t count) {
  return ff_sd_read(pdrv, buffer, sector, count) == ESP_OK;
}
...
bool sd_write_raw(uint8_t pdrv, uint8_t *buffer, DWORD sector, size_t count) {
  return ff_sd_write(pdrv, buffer, sector, count) == ESP_OK;
}
