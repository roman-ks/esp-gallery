...
bool SDFS::readRAW(uint8_t *buffer, uint32_t sector, size_t count) {
  return sd_read_raw(_pdrv, buffer, sector, count);
}
...
bool SDFS::writeRAW(uint8_t *buffer, uint32_t sector, size_t count) {
  return sd_write_raw(_pdrv, buffer, sector, count);
}
